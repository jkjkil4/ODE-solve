...
#thm-solve[
  ...
  由于 $y+3=0$（即 $y=-3$） 时，如果允许 $C = 0$，那么*特 解* $y=-3$ 也包含在上式中，所以
...
#thm-warn[这里的“齐次”含义与前面“齐次方程”中的含义是不同的]
...
#thm-method[
  考虑微分方程
  $ difyx =p(x)y^2 +q(x)y+r(x) $
  
  其中 $p(x)$，$q(x)$，$r(x)$ 均为区间 $I$ 上的连续函数，且 $p(x)!=0$，方程右边是一个关于 $y$ 的二次多项式，我们称作Riccati方程。对于这样的形式我们一般无法用之前提到的初等积分法求解，但我们已知Riccati方程有一个特解 $phi_1(x)$，我们令 $y=z+phi_1(x)$，便可以消去方程不含 $y$ 的项 $r(x)$，从而通过初等积分法求出其通解。

  一般来说，消去后得到的是 Bernoulli 方程，也有机会把 $y$ 一次项也消掉得更简单的形式。
]
#thm-example[
  求解方程
  ...
  我们首先对这个方程进行变形为
  $ y^'+e^x y^2-2e^(2x)y=e^x-e^(3x) $
  
  然后我们与上面的Riccati方程的形式进行对比，发现这是一个Riccati方程。观察发现，这个方程有一个特解 $y=e^x$，之后我们令 $y=z+e^x$，则我们就可得到一个新的关于 $z$ 的方程
  $ z^'+e^x z^2=0 $
  
  这个方程刚好也消去了 $y$ 的一次项，对这个方程我们就可以运用初等积分法得其通解为
  $ z=1/(e^x+C) $
...
#thm-explain[
  该题的最后一步也可以通过观察凑全微分来求解，例如可以把 @eq-example-3x3y2x2yx 写成
